...
public:
  I2C_BUS();
  static void scanI2C_Default(); /* Scans connection to I2C Adresses, returns them */
  static void scanI2C_OLED();    /* Scans connection to I2C Adresses, returns them */
  static void suspendTask();     /* Suspend the task after the scan is done */

private:
  static bool stopTask_i2c_default; /* Bool var for I2C scan: init as false, after the scan of devices is done -> bool = true; task is suspended */
  static bool stopTask_i2c_OLED;    /* Bool var for I2C scan: init as false, after the scan of devices is done -> bool = true; task is suspended */
};

bool I2C_BUS::stopTask_i2c_default = false;
bool I2C_BUS::stopTask_i2c_OLED = false;
...
void T_scanI2CTask(void *parameter) /* Main function of the Task */
{
  ...
  i2c_bus.scanI2C_Default(); /* Scan for I2C decices, suspend task after scan */
  i2c_bus.scanI2C_OLED();    /* Scan for I2C decices, suspend task after scan */
  i2c_bus.suspendTask();     /* Suspend the task after the scan is done */
...
/****************************************************************************************************************************/
void I2C_BUS::scanI2C_Default() /* Scans connection to I2C Adresses, returns them */
{
  ...
  stopTask_i2c_default = false; /* Bool var for I2C scan: init as false, after the scan of devices is done -> bool = true; task is suspended */

  Serial.println("--> Scanning for I2C devices on Default I2C Bus:");
  ...
  Serial.println("--> Scan complete on I2C default bus");

  stopTask_i2c_default = true;
...
/*______________________________________________________SCAN_I2C_FUNCTION___________________________________________________*/
/****************************************************************************************************************************/
void I2C_BUS::scanI2C_OLED() /* Scans connection to I2C Adresses, returns them */
{
  byte error, address;       /* Byte error for address and address of I2C interface device */
  int nrDevices;             /* Variable for nr. of devices found */
  stopTask_i2c_OLED = false; /* Bool var for I2C scan: init as false, after the scan of devices is done -> bool = true; task is suspended */

  Serial.println("--> Scanning for I2C devices on OLED I2C Bus:");

  nrDevices = 0;
  for (address = 1; address < 127; address++) /* Loop trough all posibile addresses */
  {
    Wire1.beginTransmission(address); /* Begin communication with the device at current address */
    error = Wire1.endTransmission();   /* End the communication and translate it to an error */
  ...
  Serial.println("--> Scan complete on OLED I2C bus");

  stopTask_i2c_OLED = true;
  // if (stopTask == true) /* Scan is done --> task cand be suspended */
  // {
  //   delay(500);
  //   vTaskSuspend(NULL);
...
/****************************************************************************************************************************/
/*__________________________________________________SUSPEND_TASK_FUNCTION___________________________________________________*/
/****************************************************************************************************************************/
void I2C_BUS::suspendTask() /* Suspend the task after the scan is done */
{
  if (stopTask_i2c_default == true && stopTask_i2c_OLED == true) /* Scan is done --> task cand be suspended */
  {
    Serial.println("[!!!] I2C scan task suspended after scan [!!!]");
    delay(500);
    vTaskSuspend(NULL);
  }
}
